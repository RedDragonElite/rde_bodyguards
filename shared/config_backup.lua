-- ============================================================
-- RDE | ULTIMATE BODYGUARD SYSTEM v4.0  —  shared/config.lua
...
Config.Pricing = {
    ...
    perSkillPt  = 120,
    -- rarityBonus: keyed by rarity string, in ORDER (used by rollRarity)
    rarityBonus = {
        common    = 0,
        uncommon  = 4000,
        rare      = 12000,
...
-- ────────────────────────────────────────────────────────────
-- PROGRESSION
-- BUG FIX: Lua 5.4 removed math.pow → use ^ operator instead
-- ────────────────────────────────────────────────────────────
Config.MaxLevel    = 100
...
Config.Morale = {
    startValue      = 75,
    decayPerMin     = 1,       -- integer, SQL TINYINT safe
    gainOnKill      = 8,
    gainOnProtect   = 12,
    gainOnInteract  = 15,
    penaltyOnDeath  = 30,
...
Config.Wounds = {
    ...
    medkitItem   = 'medkit',
    -- Ordered array for index lookups (none=1 → critical=4)
...
-- ────────────────────────────────────────────────────────────
-- PED MODELS  —  Maximal realistisch, keine Cops, maximale Abwechslung
-- ────────────────────────────────────────────────────────────
Config.Peds = {
    male = {
        -- Zivilisten & Alltag
        ...
        'a_m_y_methhead_01', 'a_m_y_runner_01', 'a_m_y_runner_02',
        'a_m_y_stbla_01', 'a_m_y_stbla_02', 'a_m_y_stlat_01',
        'a_m_y_stwhi_01', 'a_m_y_stwhi_02', 'a_m_y_sunbathe_01',
        'a_m_y_vindouche_01', 'a_m_y_vinewood_01', 'a_m_y_vinewood_02',
        'a_m_y_vinewood_03', 'a_m_y_vinewood_04', 'a_m_y_hipster_01',
        'a_m_y_hipster_02', 'a_m_y_hipster_03', ]]--
        ...
        -- Spezial-Peds (z. B. Mechaniker, Handwerker)
    ...
    female = {
        -- Zivilistinnen & Alltag
        'a_f_m_beach_01', 'a_f_m_bevhills_01', 'a_f_m_bevhills_02',
        'a_f_m_bodybuild_01', 'a_f_m_business_02', 'a_f_m_downtown_01',
        'a_f_m_eastsa_01', 'a_f_m_eastsa_02', 'a_f_m_fatcult_01',
        'a_f_m_genfat_01', 'a_f_m_genfat_02', 'a_f_m_ktown_01',
        'a_f_m_ktown_02', 'a_f_m_proto_01', 'a_f_m_salton_01',
        ...
        -- Spezial-Peds (z. B. Mechanikerinnen)
...
-- ────────────────────────────────────────────────────────────
-- RARITIES  — stored as ORDERED array so rollRarity is deterministic
-- BUG FIX: was pairs() on hash table = random iteration = broken weights
-- ────────────────────────────────────────────────────────────
Config.Rarities = {
...
-- Fast lookup by key (built once at load)
Config.RarityByKey = {}
...
-- ────────────────────────────────────────────────────────────
-- ACHIEVEMENTS
-- BUG FIX: field is 'desc', not 'description' — was crashing concat
-- ────────────────────────────────────────────────────────────
Config.Achievements = {
...
-- ============================================================
-- SHARED HELPER FUNCTIONS  (available on both client & server)
-- ============================================================

-- BUG FIX: math.pow removed in Lua 5.4 → use ^ operator
function Config.XPForLevel(level)
    return math.floor(Config.BaseXP * (Config.XPMult ^ (level - 1)))
end

-- Morale tier from numeric value (0-100)
...
-- Relationship tier from tier level number (1-5)
...
-- Rarity data by key string — safe lookup with fallback
function Config.GetRarity(key)
    return Config.RarityByKey[key] or Config.RarityByKey['common'] or Config.Rarities[1]
end

-- BUG FIX: was pairs() over hash → non-deterministic cumulative weights
-- Now uses ordered array
